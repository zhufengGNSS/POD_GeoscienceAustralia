...
! ----------------------------------------------------------------------
      ...
      REAL (KIND = prec_q), DIMENSION(3) :: er,e_sun,ev,en,ey,ed,ex,et
...
! --------------------------------
      Call productcross(er,ev,yy)
      en(1)=yy(1)/sqrt(yy(1)**2+yy(2)**2+yy(3)**2)
      en(2)=yy(2)/sqrt(yy(1)**2+yy(2)**2+yy(3)**2)
      en(3)=yy(3)/sqrt(yy(1)**2+yy(2)**2+yy(3)**2)
...
!PRINT*,'SUN',SUN
CALL ERPFBOXW(ERM,ANT,GRD,REFF,YSAT,SUN,KAPPA,MONTH,BLKNUM,SVN,MJD1,ACCEL)
...
! ---------------------------
   ...
   ft_erp = fx_erp*et(1)+fy_erp*et(2)+fz_erp*et(3)
   fn_erp = fx_erp*en(1)+fy_erp*en(2)+fz_erp*en(3)

!IF (abs(fr_erp-ACCEL(1)) > 1.D-11) THEN
!PRINT*,'SAT_EARTH_SUN ANGLES',psi,'ACC IN R, T, N = ',&
!       fr_erp-ACCEL(1),ft_erp-ACCEL(2),fn_erp-ACCEL(3)
!END IF
fx_erp = ACCEL(1) 
fy_erp = ACCEL(2)
fz_erp = ACCEL(3)
