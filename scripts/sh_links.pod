...
if ($#argv == 0) then
   ...
   echo ' FORMAT' 
   echo ' '
   echo '    sh_links.pod -default -gravity [static file] -ocean [tide file] -planet [ephem file] -eop [eop file] -satinfo [satinfo file]'
   echo ' '
   echo ' -default : Link default  set of files from ~/pod/tables directory to local directory'
   echo '    ln -s ~/pd/tables/ascp1950.430         ./ascp1950.430        '
   echo '    ln -s ~/pd/tables/fes2004_Cnm-Snm.dat  ./fes2004_Cnm-Snm.dat '
   echo '    ln -s ~/pd/tables/goco05s.gfc          ./goco05s.gfc         '
   echo '    ln -s ~/pd/tables/header.430_229       ./header.430_229      '
   echo '    ln -s ~/pd/tables/satinfo.dat          ./satinfo.dat         '
   echo ' '
   echo ' OPTIONAL'                                               
   echo ' '        
   ...
   echo '    -satinfo = satellite info file to link' 
   echo ' '
   echo ' EXAMPLES'                               
   echo '    sh_links.pod -d'
...
if ( ! -e satinfo.dat         )    \rm -r satinfo.dat         >& /dev/null       

# Make new set of default lionks links if necessary
ln -s ~/pod/tables/ascp1950.430         ascp1950.430        >& /dev/null
ln -s ~/pod/tables/fes2004_Cnm-Snm.dat  fes2004_Cnm-Snm.dat >& /dev/null
ln -s ~/pod/tables/goco05s.gfc          goco05s.gfc         >& /dev/null
ln -s ~/pod/tables/header.430_229       header.430_229      >& /dev/null
ln -s ~/pod/tables/satinfo.dat          satinfo.dat         >& /dev/null     
...
if (${eop} != '') then
  ln -s ~/pod/data/${eop}            ${eop}          >& /dev/null 
endif
